...
contract MockYearnVault is ERC20, MockERC20 {
  ...
  // }
  function setPricePerShare(uint256 _amount) external {
    _pricePerShare = _amount;
  }
  ...
  function deposit(uint256 _amount) external returns(uint256) {
    IERC20(underlying).transferFrom(msg.sender, address(this), _amount * 10**18 / _pricePerShare);
    _mint(msg.sender, _amount * 10**18 / _pricePerShare);
    return _amount * 10**18 / _pricePerShare;
  }

  function withdraw(uint256 _amount) external returns(uint256) {
    uint256 toRedeem = _amount * _pricePerShare / 10**18;
